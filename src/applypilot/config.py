...
def get_tier() -> int:
    ...
    Tier 3 (Full Auto-Apply):       + Claude Code or Codex CLI + Chrome
    """
    load_env()
    ...
    has_browser_agent = any(shutil.which(name) for name in ("claude", "codex"))
    try:
        get_chrome_path()
        has_chrome = True
    ...
    if has_browser_agent and has_chrome:
        return 3
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 3:
        if not any(shutil.which(name) for name in ("claude", "codex")):
            missing.append("Claude Code or Codex CLI")
        try:
            get_chrome_path()
        except FileNotFoundError:
